...
COLUMN_ALIASES = {
    ...
    "collection_date": {"collection_date", "fecha_colecta", "fecha_de_colecta", "fecha"},
    ...
    "seed_quantity": {"seed_quantity", "cantidad_semillas", "semillas", "cantidad"},
    "available": {"available", "disponible", "disponibilidad"},
    "storage_location": {"storage_location", "ubicacion_almacenamiento", "ubicación_almacenamiento"},
    "observations": {"observations", "observaciones", "notas", "notes"},
}
...
def canonical_columns(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    return frame.rename(columns={column: alias_lookup.get(normalize_header(str(column)), column) for column in frame.columns})
...
def require_columns(frame: pd.DataFrame) -> None:
    missing = [column for column in ("accession_code", "scientific_name") if column not in frame.columns]
    if missing:
