...
Reemplazá `https://your-project.supabase.co` y `your-service-role-key` por los valores reales de **Supabase → Project Settings → API**. En macOS usá `python3` para crear el entorno si `python` no existe. Si `pip install` se cancela o falla, el script no va a encontrar el paquete `supabase`; volvé a ejecutar `pip install -r scripts/requirements.txt` dentro del entorno virtual.

La guía completa está en [`docs/import-data.md`](docs/import-data.md), incluyendo columnas esperadas, aliases como `ID` para el código de accesión, cómo obtener credenciales, cómo importar desde CSV/Excel, errores comunes y cómo verificar la carga en Supabase.
